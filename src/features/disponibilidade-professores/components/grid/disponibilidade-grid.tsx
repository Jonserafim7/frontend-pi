...
export const DisponibilidadeGrid: React.FC<DisponibilidadeGridProps> = ({
  ...
  // Processar dados para formar grid
  const gridData = useMemo(() => {
    console.group("🔍 DEBUG GRID DISPONIBILIDADES")
    console.log("📊 slotsValidos recebidos:", slotsValidos)
    console.log("📋 disponibilidades recebidas:", disponibilidades)

    if (!slotsValidos?.slots) {
      console.log("⚠️ slotsValidos.slots está vazio ou undefined")
      console.groupEnd()
      return []
    }

    const slots = slotsValidos.slots
    const grid: SlotData[] = []

    console.log("🔍 Processando slots:", slots)
    ...
    DIAS_SEMANA.forEach((dia) => {
      slots.forEach((slot: any) => {
        ...
        // Verifica se há uma disponibilidade que contém ou intersecta com este slot
    ...
    console.log("🗂️ gridData processado:", grid)
    console.groupEnd()
    return grid
  }, [slotsValidos, disponibilidades])

  // Agrupar por horário para as linhas
  const slotsByTime = useMemo((): TimeSlotGroup[] => {
    if (!slotsValidos?.slots) return []

    const result = slotsValidos.slots.map(
      (slot: { inicio: string; fim: string }): TimeSlotGroup => ({
    ...
    console.log("⏰ slotsByTime processado:", result)
    return result
  }, [gridData, slotsValidos])

  // Handler para click em slot
  const handleSlotClick = async (slot: SlotData) => {
    console.group("👆 SLOT CLICK DEBUG")
        ...
        <CardContent>
          <div className="flex h-64 items-center justify-center">
            <div className="text-center">
              ...
              <p className="text-muted-foreground">Carregando grid...</p>
            </div>
          </div>
        </CardContent>
      </Card>
        ...
        <CardContent>
          <div className="py-8 text-center">
            ...
            <p className="text-muted-foreground">
              Os horários disponíveis ainda não foram configurados pelo diretor.
            </p>
          </div>
        </CardContent>
      </Card>
        ...
        <div className="overflow-x-auto">
          <div className="min-w-[600px]">
            ...
            <div className="mb-4 grid grid-cols-8 gap-2">
              <div className="text-muted-foreground flex h-12 items-center justify-center text-sm font-medium">
                Horário
              </div>
              {DIAS_SEMANA.map((dia) => (
                <div
